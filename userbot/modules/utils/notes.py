...
# from userbot.models import Note, Chat
...
#         chat = None
#         if args.get('local'):
#             chat = Chat.objects(chat_id=event.chat.id).get()
...
#         if note.chat and note.chat.chat_id != event.chat.id:
#             await log_message(f"Note `{name}` is local to chat `{note.chat}`.")
#             return

#         if note.file and note.file.strip() != '':
#             await event.client.send_file(event.chat.id, note.file, caption=note.content)
#         else:
